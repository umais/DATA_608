...
tooltip = GeoJsonTooltip(
    fields=["name", "coal", "solar", "wind", "hydro", "total"],
    aliases=["State:", "Coal:", "Solar:", "Wind:", "Hydro:", "Total:"],
    localize=True,
    sticky=True,
    labels=True,
...
# === STATIC MAP (PDF) ===
# Now we'll plot using the original CRS (EPSG:4326) without any projection distortions
...
# Annotate state names (adjusted for proper placement)
for _, row in merged.iterrows():
    txt = ax.text(
...
# Save static map to PDF (using bbox_inches='tight' to avoid excessive margins)
